...
const AdminList = () => {
  ...
  const filtered = members
    ...
    .filter((m) => (search ? m.fullName?.toLowerCase().includes(search.toLowerCase()) : true));
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Daftar peserta (Admin)</h2>
          <p className="text-sm text-gray-400 mt-0.5">{filtered.length} peserta</p>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">No.</th>
                  ...
                  <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide w-24">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                  ...
                  <tr key={m._id} className="hover:bg-gray-50 transition">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 ${m.gender === "M" ? "bg-blue-100 text-blue-700" : "bg-pink-100 text-pink-700"}`}>
                          {m.fullName?.split(" ").slice(0, 2).map((w) => w[0]).join("")}
                        </div>
                        <span className="font-medium text-gray-800">{m.fullName}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-gray-500 font-mono text-xs">{m.number}</td>
                    <td className="px-5 py-3">
                      {m.religion ? (
                    ...
                    <td className="px-5 py-3 text-gray-500 max-w-[160px] truncate">{m.address || "—"}</td>
                    <td className="px-5 py-3">
                      <button
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-12 text-gray-400">Tidak ada data</td>
                  </tr>
                )}
              </tbody>
            </table>
