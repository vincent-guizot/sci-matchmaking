        ...
        {data.map((p, i) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {p.religion && (
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${RELIGION_COLORS[p.religion] || "bg-gray-100 text-gray-600"}`}
                >
                  {p.religion}
                </span>
              )}
...
const MostLiked = () => {
  ...
  const allRanked = Object.values(likeCountMap).sort(
    (a, b) => b.count - a.count,
  );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          {
            label: "Total semua likes",
            value: totalLikes,
            color: "text-rose-500",
          },
          {
            label: "Total likes pria",
            value: totalMenLikes,
            color: "text-blue-500",
          },
          {
            label: "Total likes wanita",
            value: totalWomenLikes,
            color: "text-pink-500",
          },
        ].map((s) => (
          <div
            ...
            <div className="bg-gradient-to-br from-pink-50 to-white border border-pink-100 rounded-2xl p-5 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-pink-400 font-medium uppercase tracking-wide">
                  Wanita paling disukai
                </p>
                <p className="font-bold text-gray-800">
                  {womenRanked[0].fullName}
                </p>
                <p className="text-rose-500 font-semibold">
                  {womenRanked[0].count} likes
                </p>
              </div>
              <span className="ml-auto text-3xl">🥇</span>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-gray-100 bg-gray-50">
            <h3 className="font-semibold text-sm text-gray-700">
              Semua peserta — ranking gabungan
            </h3>
          </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {enriched.map((p, i) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${p.gender === "M" ? "bg-blue-100 text-blue-700" : "bg-pink-100 text-pink-700"}`}
                      >
