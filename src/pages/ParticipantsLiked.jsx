...
const ParticipantsLiked = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
      {/* Header */}
      <div>
        <h2 className="text-xl font-bold text-gray-900">Participants yang sudah memilih</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Peserta</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Pilihan</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide w-24">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {likesData.map((row) => (
                  <tr key={row._id} className="hover:bg-gray-50 transition align-top">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        >
                          {row.fullName?.split(" ").slice(0, 2).map((w) => w[0]).join("")}
                        </div>
                        <div>
                          <p className="font-semibold text-gray-800">
                            {row.number}{row.gender === "M" ? "M" : "W"} — {row.fullName}
                          </p>
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${row.gender === "M" ? "bg-blue-100 text-blue-700" : "bg-pink-100 text-pink-700"}`}>
                    ...
                    <td className="px-5 py-4">
                      {row.likes.length === 0 ? (
                        <span className="text-gray-300 text-xs">Belum memilih</span>
                      ) : (
